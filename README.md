# Polyglot Engineer Templates

[![License: MIT](https://img.shields.io/badge/License-MIT-yellow.svg)](https://opensource.org/licenses/MIT)
[![Templates](https://img.shields.io/badge/templates-5-blue.svg)](justfile-templates/)
[![GitHub stars](https://img.shields.io/github/stars/dr-saad-la/polyglot-engineer-templates.svg?style=social&label=Star)](https://github.com/dr-saad-la/polyglot-engineer-templates)
[![GitHub forks](https://img.shields.io/github/forks/dr-saad-la/polyglot-engineer-templates.svg?style=social&label=Fork)](https://github.com/dr-saad-la/polyglot-engineer-templates/fork)
[![Justfile Validation](https://github.com/dr-saad-la/polyglot-engineer-templates/actions/workflows/validate-justfiles.yml/badge.svg)](https://github.com/dr-saad-la/polyglot-engineer-templates/actions/workflows/validate-justfiles.yml)
[![Issues](https://img.shields.io/github/issues/dr-saad-la/polyglot-engineer-templates)](https://github.com/dr-saad-la/polyglot-engineer-templates/issues)
[![Pull Requests](https://img.shields.io/github/issues-pr/dr-saad-la/polyglot-engineer-templates)](https://github.com/dr-saad-la/polyglot-engineer-templates/pulls)
[![Code of Conduct](https://img.shields.io/badge/code%20of%20conduct-contributor%20covenant-purple.svg)](CODE_OF_CONDUCT.md)
[![Security](https://img.shields.io/badge/security-policy-blue.svg)](SECURITY.md)


**Production-ready templates for The Polyglot Engineer ecosystem.**

This repository is part of a three-repo ecosystem:
- [polyglot-engineer-blog](https://github.com/dr-saad-la/polyglot-engineer-blog) - Main blog
- [polyglot-engineer-code-examples](https://github.com/dr-saad-la/polyglot-engineer-code-examples) - Code examples
- **polyglot-engineer-templates** - Reusable templates (this repository)

---

## 📦 Template Categories

### 🔧 Justfile Templates (v1.0.0)

Task automation templates for different project types. Download, customize, automate.

**Available Templates:**
- [Python](#python) - General Python projects
- [FastAPI](#fastapi) - Web APIs and services
- [CLI](#cli) - Command-line applications
- [ML/Data Science](#mldata-science) - Machine learning projects
- [General](#general) - Minimal starter template

[**Browse Justfile Templates →**](justfile-templates/)

---

### 🐳 Docker Templates (Coming Soon)

Container configurations for different environments.

### ⚙️ GitHub Actions Templates (Coming Soon)

CI/CD workflow templates for automated deployments.

### 🐍 Python Project Templates (Coming Soon)

Complete project scaffolding with best practices.

### 🌐 API Templates (Coming Soon)

REST and GraphQL API boilerplates.

---

## 🚀 Quick Start

### Justfile Templates

#### Python
````bash
curl -o justfile https://raw.githubusercontent.com/dr-saad-la/polyglot-engineer-templates/main/justfile-templates/python/justfile
````

#### FastAPI
````bash
curl -o justfile https://raw.githubusercontent.com/dr-saad-la/polyglot-engineer-templates/main/justfile-templates/fastapi/justfile
````

#### CLI
````bash
curl -o justfile https://raw.githubusercontent.com/dr-saad-la/polyglot-engineer-templates/main/justfile-templates/cli/justfile
````

#### ML/Data Science
````bash
curl -o justfile https://raw.githubusercontent.com/dr-saad-la/polyglot-engineer-templates/main/justfile-templates/ml/justfile
````

#### General
````bash
curl -o justfile https://raw.githubusercontent.com/dr-saad-la/polyglot-engineer-templates/main/justfile-templates/general/justfile
````

### Customize & Use
````bash
# 1. Edit configuration variables
vim justfile

# 2. See available commands
just

# 3. Setup project
just setup

# 4. Start development
just dev
````

---

## 📚 Documentation

- [Getting Started Guide](docs/guides/getting-started.md)
- [Customization Guide](docs/guides/customization.md)
- [Best Practices](docs/guides/best-practices.md)
- [Template Examples](docs/examples/)

---

## 🤝 Contributing

Contributions welcome! See [CONTRIBUTING.md](CONTRIBUTING.md)

**Ideas for templates:**
- Docker multi-stage builds
- GitHub Actions workflows
- Terraform configurations
- Kubernetes manifests

---

## 📄 License

MIT License - see [LICENSE](LICENSE)

---

## 📧 Contact

**Dr. Saad Laouadi**

- Email: dr.saad.laouadi@gmail.com
- LinkedIn: [saad-laouadi](https://linkedin.com/in/saad-laouadi)
- GitHub: [dr-saad-la](https://github.com/dr-saad-la)
- Blog: [The Polyglot Engineer](https://dr-saad-la.github.io/polyglot-engineer-blog)

---

## ⭐ Support

If you find these templates helpful:
- ⭐ Star this repository
- 🔗 Share with colleagues
- 💡 Contribute improvements
- 🐛 Report issues

---

**Made with ❤️ by Dr. Saad Laouadi | Part of The Polyglot Engineer Ecosystem**
